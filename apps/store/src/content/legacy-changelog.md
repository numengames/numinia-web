...
>
> Entry types: **NEW** (feature) · **FIX** · **UPD** (update/improvement).
...
- UPD — Season types with puzzle types, duration, difficulty, descriptions (EN + JA)
- UPD — Graceful degradation: Stripe, Thirdweb, NFT mint degrade independently
- UPD — Best-effort NFT minting pattern (pass works via JSON even if mint fails)
- UPD — LAPShell uses Thirdweb ConnectButton when configured (fallback to legacy)
...
- UPD — 190 tests across 19 files (was 158/18)
...
- UPD — i18n: complete coverage for gallery, actions, viewer, and finder components
...
- UPD — Skeleton screens replace spinner loading states across the platform
- UPD — Gallery card micro-interactions (hover scale, shadow transitions)
- UPD — Blur placeholder (LQIP) replaces animate-pulse for image loading
- NEW — Error pages (404, 500) with branded design + global-error boundary
- UPD — Accessibility: improved touch targets and keyboard navigation
...
- UPD — PWA manifest improvements + visual consistency polish
...
- UPD — Health endpoint upgraded — checks GitHub API + R2 CDN with latency
- NEW — Privacy Policy — international data transfers (GDPR Art. 44-49) documented
- UPD — Cookie consent redesigned — floating card, accept/reject, auditable record
- UPD — Thumbnails migrated to Next.js <Image> — WebP/AVIF, responsive srcset
...
- UPD — Cache headers: stale-while-revalidate on assets, portals, collections API
...
- UPD — CI: npm audit now blocking (was continue-on-error)
- NEW — prefers-reduced-motion — disables all animations (WCAG 2.1)
- UPD — llms.txt expanded — FAQ, all API endpoints, AI agent instructions
- UPD — next.config.js — 9 new remotePatterns (R2, Arweave, IPFS domains)
...
- UPD — SEO: sitemap updated, hreflang, canonical URLs, home page SSR
- NEW — AI SEO: llms.txt + OpenAPI docs (/openapi.json)
- UPD — Accessibility: aria-labels on all viewers, skip-to-content, focus trap
- NEW — Health check endpoint: GET /api/health
- UPD — CI: GitHub Actions pinned to SHA (supply chain security)
- NEW — Backfill hashes script (SHA-256 for all assets)
- UPD — Three.js skipped on mobile (saves ~2MB)
...
- UPD — Dice roll buttons always visible on character stats
- NEW — Portals: interactive steampunk world map (4 districts, 14 portals)
- UPD — 158 tests across 18 files
...
- UPD — Loot section (renamed from Digital Goods)
- UPD — Sidebar reorganized: Character → Portals → Loot → Assets → Archive
- UPD — Upload merged into Assets view (inline panel)
...
- UPD — 22 Resources pages rewritten with Numinia content
- UPD — Favicon unified with numen.games (Khepri)
...
- UPD — 138 tests across 15 files (was 63)
- UPD — 0 process.env bypasses (all through env.ts Zod)
...
- UPD — Favicon unified with numen.games (Khepri icon)
- UPD — 22 Resources pages rewritten with Numinia content
...
- UPD — Unified wallet login design (gallery = admin)
...
- UPD — Asset count moved to filter bar
...
- UPD — View toggle moved next to filter buttons
- UPD — Tooltips on admin UI elements
- FIX — Debug endpoint protected
- FIX — .env.example updated with all variables
- UPD — Storage/status/version/NFT fields passed to frontend
...
- UPD — Upload feedback: active layer + asset ID
- FIX — Audio/Video/Worlds visible in gallery sidebar
- UPD — PageSpeed: deferred hero VRM load
...
- UPD — Upload progress bar

## v0.3.0 — 2026-03-30 11:00

- UPD — PreviewPanel split (1943→1025 lines)
- UPD — 44→1 any types eliminated
- UPD — 5 JSX→TSX migrated
- UPD — GitHub API in-memory cache
