...
import { Save, Download, FileText, Pencil, Eye, Loader2, Camera, Dices } from 'lucide-react';
      ...
      <div className="flex items-center gap-2">
        ...
        {value > 0 && (
          <button
            ...
            className="p-1 rounded-md text-amber-400/60 hover:text-amber-500 hover:bg-amber-500/10 transition-all"
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 print:p-0 print:max-w-none">
      ...
      <div className="space-y-4 print:space-y-2">
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 print:border print:rounded-none">
          <div className="flex gap-5">
            {/* Profile pic — click to open crop modal */}
            <div className="shrink-0">
              <div
                className={`relative h-24 w-24 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 group ${editing ? 'cursor-pointer ring-2 ring-transparent hover:ring-violet-400/50' : ''} transition-all`}
                onClick={() => editing && setShowAvatarCrop(true)}
              >
                {char.profile_image ? (
                  <img src={char.profile_image} alt={char.name} className="w-full h-full object-cover" draggable={false} />
                ) : (
      ...
      <AvatarCropModal
        ...
        onSave={(croppedDataUrl) => setChar(prev => ({ ...prev, profile_image: croppedDataUrl }))}
      />
    </div>
  );
}
