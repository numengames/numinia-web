...
> **In the system.** Observes: every merge and Oracle order. Regulates: priorities. Coupled to: TODO.md, missions (numinia-nwos MIS-072+).
>
> _Part of the Law. Index: [LEY.md](./LEY.md)_

> Living document: what is DONE, what is NEXT, and who owns each next step.
> Updated: 2026-08-16 · remote LIVE: `numengames/numinia-web` (**PUBLIC** since 2026-08-16 — every MIT declaration on main is a live grant) · **numinia.com SERVES THE PLATFORM** (deployed via GitHub Actions) · licensed per canon C-005 (ADR-019).

## Where we are

**LIVE on numinia.com** (the exact deployed commit is always at numinia.com/version.json) — code on `numengames/numinia-web` (PUBLIC), auto-deploy on green CI, every push that passes ships itself (docs/deploy-runbook.md).
...
- Legacy test audit: 3 severe security findings, 12 binding test rules → all encoded into this repo's machinery. Report deleted 2026-09-11 once spent (nwos:ADR-030); git holds it at 1014349.
...
- **Night watch (2026-08-16, "quemar tokens hasta que despierte") ✅**: MISSION-016 COMPLETE (v0.32.0) — the census desk in the management zone (5 locales, honest 403/503/absence states, Oracle absent from the grant options; lights up with zero code changes when D23 lands). Attestations shipped in @numinia/auth (ADR-018 option C: na1-prefixed HMAC statements with domain separation from sessions, pinned both directions; no expiry; Oracle not attestable). Mutation hardening: @numinia/state 81%→98% (25 real gaps killed), attestation.ts 98.5% (1 documented equivalent).
- **Night watch, second half ✅ (2026-08-16 dawn)**: Khepri v4.2.0 read WHOLE + §19.4 document-level audit (one violation in the entire site: census double primary — fixed; one sweep max and Coral/Grana separation verified mechanically). Keyboard e2e joins the gates (tab order + the §5 focus ring actually rendering). **The RPG manual read WHOLE** (agent-delegated) → `docs/reference/manual-map.md`: chapter anchors, hard-fact registry, named-places census, and 7 provenance divergences documented (the 6-rank ladder and named thresholds are PLATFORM inventions, not manual canon; **D24 opened**: data repo vs manual disagree on the Temple of Khepri's district). Canon corrected the portals on contact: «La Casa de la Moneda» (never «La Ceca»), SIX Ontological Schools. WebGL e2e margins hardened after a load-starved flake. Everything deployed itself as it landed.
- **Dawn continuation ✅ (2026-08-16 early morning, "continúa")**: runbook item 12 CLOSED — visual baselines regenerated inside the CI image via podman (they matched the local set within the 2% ratio: one truth for both worlds), skip guard lifted, the pixel gate now runs remotely. Provenance annotations landed in rank.ts/seal.ts per the manual map. **The city got its calling card**: default og:image + twitter card on every page (Noche + relieve + wordmark + 1920·2020·2120 + Khepri, generated from kit assets); asset thumbnails pass through a BaseLayout prop. uuid moderates: deliberately NOT overridden (deep-import risk inside the wallet vendor chain outweighs a moderate advisory; gate threshold is high and clean).
- **PERFORMANCE program ✅ (v0.33.0, 2026-08-16 morning — MISSIONS 017–020, Oracle order "céntrate en performance")**: board groomed to reality first. Then measurement-first: /archive/ shipped 2.4MB of full-res images into 240px cards → thumbnail pipeline bakes 480px WebPs from fixtures (scripts/build-thumbs.mjs, fixture doctrine) → **archive 2,418→469KB (−81%), gallery 1,717→318KB (−81%), verified LIVE on numinia.com** (651ms networkidle). Perf GATE added (per-page weight budgets, zero-JS promise on Layer 0/1, CLS<0.1). Sheet island stopped smuggling the 5-locale domain to the browser (id-only validation, 120→80KB). Fonts preloaded; `_headers` cache policy live-verified on Workers assets; Playwright workers capped (16 Chromiums starved software WebGL on 32 cores). **Root-caused the recurring 3D flake: hydration race** — islands now announce `data-hydrated` and the suite waits for the beacon (6 tests, 2.7s, deterministic). Session page (820KB wallet vendor) documented as the one sanctioned heavy page, isolated and under 1MB.
- **OPS + SECURITY programs ✅ (v0.34.0, 2026-08-16 morning — M-021→028, Oracle order)**: Workers Logs live (structured JSON per API request, unified 500 handler, wallet-truncated, no PII) · browser error beacon → /api/telemetry (capped, garbage-swallowing) · **night watch**: hourly live probes that open/close their own GitHub issue — the 3 AM scenario covered · **the walls**: CSP + HSTS + nosniff + permissions/referrer policy, dual-applied (_headers for statics, middleware for SSR), session pages relaxed for wallet vendors, live-asserted by the deploy smoke test · **the doorman**: per-IP sliding-window rate limits on login/SIWE/census/admin/telemetry with honest per-isolate caveats (KV upgrade queued with D23) · **the rotation lever**: dual-secret session verification in @numinia/auth (only a signature miss consults the previous secret; auth still 100/100) — rotating AUTH_SESSION_SECRET is now graceful global revocation · **key-rotation runbook** chartered into the Law (docs/key-rotation-runbook.md): the Oracle's 20-minute guided session, quarterly cadence. Remaining human step: EXECUTE the rotation (runbook step 1 kills the inherited deploy token).
- **MISSION-029 — Automatic preferences ✅ (v0.35.0, Oracle proposal)**: mode hierarchy stored>OS>Diurno with live sunset-following while unstored; language auto-detect in its only safe corner (first visit, root only, client-side, stored immediately); menu choice = law (D17). 5 e2e scenarios pin the hierarchy and its fights; landing inline budget raised 8000→9000B with written reason (the raise doctrine now lives in the budget file).
- **Oracle's four missions ✅ (v0.36.0, 2026-08-16 midday)**: (1) **3D FIXED in production** — R2 sends no CORS so the viewer died cross-origin since deploy day (fixtures blinded every gate); chain binaries now stream through /api/media (host-allowlisted, edge-cached, doorman-limited); the ROOT fix stays with the Oracle: a CORS policy on the R2 bucket (dash → R2 → bucket → Settings → CORS: allow https://numinia.com + www + http://localhost:4321). (2) **Responsive: 138 pages, 0 overflow at 360px** (was 47) — five root causes fixed, worst offenders pinned in the cross-engine gate. (3) **Motion**: catalog coverage extended — elevation now reaches cards-inside-links (silently missing for months), waiting dots (§10.1-07) debut on the census buttons; tecleo/moon/reveal verified already placed. (4) **Words**: a copy audit (~40 findings, NOTHING edited — canon respected; report deleted 2026-09-11 once its findings were logged, git holds it at 1014349; highlights: L.A.P. never expanded, "Conejos Legales" likely canon typo, Fase/D23/ADR leaking to citizens, wallet⇄cartera drift) + docs/conceptual-glossary.md chartered into the Law: the 25 foundational terms, each with description, epistemic value, pragmatic value and synonyms — RPG terms deliberately excluded.
- **Oracle's morning list ✅ (v0.38.0, 2026-08-16 morning, phone screenshots)**: (1) **Viewer stretch dead** — `renderer.setSize(480,480,false)`: the inline height had been overriding the responsive CSS (width shrank, height stayed 480px → stretched models); canvas now pins `aspect-ratio: 1/1`. (2) **Updates page honest** — every hour re-derived from its sealing commit in TRUE UTC (old ones were local guesses labeled UTC; git log is the source, +02:00 stripped), mission/ADR ids as chips per version, one §9.4 card per version, and by Oracle ruling the changelog vocabulary is standard English in all five locales (Updates / Latest / Fix / New / Improved / Planned). (3) **Docs healed** — dead legacy links replaced everywhere (PabloFMM/numinia-digital-goods repo, numinia.store admin, @numinia_store X handle → numengames org + data repo), developer pages tell the Astro story instead of Next.js 16, markdown tables dressed (hairline grid, amber Mono headers). (4) **One veil for all the art** — the nine city images normalized to one luminance band (sharp normalise 1–99% + measured linear lift for city/cosmos/reader; sharp file cache disabled when verifying — it silently returns pre-write stats). (5) **City chapter nav** = one scrolling row on phones (wrapped rows rode over the text). (6) **thirdweb widget wears Khepri** in both modes via `theme` prop + a MutationObserver on `html[data-modo]` (apps/store/src/components/auth/widget-theme.ts — palette mirrored from the kit, never invented).
- **Lore runbook + login day ✅ (v0.39.1–v0.39.2, 2026-08-17 morning)**: (1) **ADR-020 runbook EXECUTED** — numinia-web private → private `numengames/numinia-lore` created + pushed (both lineages) → `LORE_TOKEN` fine-grained PAT sealed as Actions secret → clean history force-pushed (`11adc23` → `9c93346`); **C-005 §4 visibility gate PASSED** against the real listing, Oracle ruling: manual-map.md stays; repo cleared to return to public (Pablo's click pending). (2) **Login FIXED end-to-end, three stacked causes**: SIWE domain hardcoded to numinia.store → `resolveAuthDomain` host allowlist, fail-closed (v0.39.1) · ROOT CAUSE: the Cloudflare build statically freezes every `process.env` expression (even `globalThis.process.env`) to `{}` — auth AND the census store could never read Worker secrets; env now flows via middleware priming from `import { env } from 'cloudflare:workers'` (v0.39.2, lib/runtime-env.ts; Astro 7 removed locals.runtime.env — its getter throws) · the Worker's `THIRDWEB_SECRET_KEY` value was wrong — re-put from apps/store/.env; `AUTH_SESSION_SECRET` rotated (graceful, dual-secret lever already shipped). Live-verified: login GET returns a real SIWE payload with `domain: numinia.com`. (3) **Deploy smoke test hardened** — a hollow 401 no longer passes: it must say "No session" and the login payload endpoint must answer 200 (the old check green-lit an unconfigured auth since deploy day). Remaining: Pablo's real-wallet sign-in at /lap/session/.
- **MIS-085 phases A+B — the Codex opens ✅ (v0.41.0, 2026-08-17)**: the v0.6.0 manual replaces ALL previous codex content. **Phase A (numinia-lore)**: edition matter committed there — glossary distilled (79 terms, every definition sourced from the manual, unsourceable flagged), Hoja de PJ v0.6.0 image + structured transcription, provisional legal note (D10), canonical acknowledgments; the manual stays ONE canonical file (Oracle amendment: build-time splitting only). **Pipeline**: splitManual reconstructs the source byte-exactly (the sacred invariant, tested), stable block anchors, Zod manifest; fetch-lore brings manual + edition docs with per-file markers, fixtures keep hermetic builds honest. **Phase B (the reader)**: the book plane per the frozen Diurno mockup — Alegreya self-hosted (D12), paper grain background-only, chrome that steps aside on scroll, self-drawing corner frames, portada/índice/capítulos/glosario/colofón, the Umbral veils chapters 2+ in the READER only (teaser shipped, body absent — never CSS-hidden), free loginless .md downloads whole-book + per-chapter (D6), bookmark D3 schema + manual-own Diurno/Nocturno (D13) + text size in localStorage, lunar progress growth-only, print stylesheet, legacy capitulo-N URLs redirect to title slugs (D8). Legacy reader (CodexGate/ManualChapter/ManualIndex + typographic parser) retired. WCAG gate caught the tertiary ink at 3.7:1 → AA-floor tokens on both papers, both modes. 118 unit + 172 e2e green. **Phase E note**: already substantively done — chapter pages are SSR and `viewerFrom(cookies)` unveils gated chapters for any SIWE session.
- **MIS-085 phase C — the book travels ✅ (v0.42.0, 2026-08-17 night)**: PDF + EPUB editions, free and loginless (D6), baked by `scripts/build-exports.mjs` into `/descargas/` on every build (local verify, CI e2e job, deploy). One engine: esbuild bundles the site's own codex TS modules for Node — no parallel parser can drift from the sacred text. PDF = whole book under codex.css `@media print` (Diurno, A4, running headers + page numbers, ~413 pages, visually reviewed page by page); EPUB = hand-built zip (no pandoc; mimetype stored first), per-chapter nav, Alegreya embedded per OFL, reserved-rights metadata (D10), cover screenshotted from the same portada — **epubcheck 5.1.0: 0 errors 0 warnings**, gated in CI (Java + pinned release). The editions surfaced real site bugs, fixed at the root: the manual's table cells carried literal `<br>` markers (now whitelisted, self-closed for XHTML), wide tables overflowed print (fixed layout + anywhere wrap), print palette had the pre-AA tertiary ink. **The brief's one mandatory question was asked**: the Oracle chose to publish with «Alicia» a secas — canon updated in numinia-lore, editions regenerate when the surname arrives. **Remaining MIS-085**: D character sheet v0.6.0 rules (audit domain types first; hoja-de-personaje.md already transcribed in numinia-lore) · F final art (Oracle delivers) · Khepri changelog row for the book-plane extension (agent drafts, Oracle commits).
- **MIS-085 phase D — the sheet learns the rules ✅ (v0.43.0, 2026-08-18 night)**: the character sheet speaks v0.6.0. **Domain**: creation facts land as data (guild/faction/species competence triads, 16-pt attribute buy, per-position +1/Umbral/Iniciativa and the full 4-axis compatibility matrix) with two resolvers (`enabledCompetences`, `positionAffinity`) — every cell verified by a SECOND independent extraction (105 position checks + 13 triads, 0 mismatches); domain still 100%, mutation 97.5% with the single survivor proven equivalent and documented. **Engine** (`lib/lap/rules.ts`): pure derivation over the sheet — budgets, enabling union, affinity pool, Aliento del Velo = Percepción; audits imported files instead of amputating them. **UI**: gears 0–5 as radio groups (Khepri §7.5, quarter-turn on set), disabled competences are real disabled controls, budget lines honest, Prestigio blank until it exists, Aptitudes Especiales with their pool, incompatible Posición flagged not blocked. **D11 sovereignty**: autosave to this device (stored copy IS the portable .md), sovereignty copy updated in 5 locales, destructive doors ask in Grana. Sheet format v3 (v2 imports untouched). New cross-engine e2e (enabling + reload persistence); character JS budget 280→300KB with written reason. **The manual's 14 gaps/contradictions the extraction surfaced** (competence cap 6 vs 5 pips, Arquetipo never defined, mestizaje undefined, «Intelecto», orphaned Iniciativa…) are catalogued for the Oracle in docs/reference/sheet-rules-findings.md — implemented only what the manual states, invented nothing. **Remaining MIS-085**: F final art (Oracle delivers) · Khepri changelog row signature (draft ready in docs/design/) · Oracle rulings on the findings doc.
- **Night watch 2026-08-18 («itera y corrige») ✅ (v0.44.0–v0.45.0)**: the Oracle's phone screenshot unmasked a THREE-RED CI streak silently skipping deploys — root-caused in two layers: WebKit-only voice-stack contact (narrator now strictly lazy) and, deeper, the hermetic-fixture corpus whose chapter slugs differ from production's (tests now resolve chapters FROM the index; reproduced via podman + the CI image, the debugging path of record). Shipped once green: **el Narrador** (Web Speech, block-by-block with solar-rule highlight, pace control; now also reads the glossary entry-by-entry), **glossary term links** in chapters (first mention, longest-form-wins; EPUB edition too), **the truly-fixed compact chrome** (the grain-stacking rule had silently out-specified `position:fixed` since the plane shipped — the bar lived only at page top; now 54→40px compact-on-read, never gone), **footer chapter arrows**, **end-of-page bookmark fix** (closing paragraphs were unmarkable), **mobile Umbral CTA with words** (was an empty pill), **la Ficha imprimible (§4.9)** — annex page ×5 locales + .md + print + inside both editions, with the render engine learning the transcription's filetes without touching the manual's path — and **REUSE 3.3 compliance for the first time, immediately armed as a CI gate**. The updates timeline caught up (cards v0.41–v0.45; it had been mute since v0.40 — changelog cards are part of the release ritual now). Codex weight pinned in the perf gate (~706KB cover, all cacheable type; 6KB JS). Overflow sweep now walks every real chapter (veiled included) instead of a hardcoded list that CI silently 302'd.
- Next: MIS-085 findings rulings + phase F art (both Oracle-gated); platform work continues on the Oracle's punch list.
...
**Runbook executed 2026-08-17** (see the dated entry above): history force-pushed,
§4 gate passed and recorded in ADR-020; the repo may return to public.

## NEXT (groomed 2026-08-16 morning — board reconciled to reality)

Retired from this table because they HAPPENED: push day (#old-1, 2026-08-15), deploy day core (#old-3 — numinia.com serves the platform via auto-deploy), portals (#old-5 → MISSION-012), the numinia.com merge (#old-11 — de facto complete, D18's EN root is live), WebKit verification (runs in CI since the first remote e2e).

| #   | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Owner                  | Blocks / notes                                                                 |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ------------------------------------------------------------------------------ |
| 1   | **Key-rotation audit** (inherited CLOUDFLARE_API_TOKEN repo secrets, R2, ~21 stale legacy Vercel vars) + decommission — `AUTH_SESSION_SECRET` and `THIRDWEB_SECRET_KEY` were re-put fresh on the Worker 2026-08-17                                                                                                                                                                                                                                                                                                                                                                     | 🧬🤖                   | The deploys run on an inherited token — the standing risk                      |
| 2   | **D23 + census ignition**: create private state repo + fine-grained PAT + `STATE_*` Worker secrets → census/ranks live → moderation → upload flow → Session Zero (D4)                                                                                                                                                                                                                                                                                                                               | 🧬 repo+token, 🤖 code | ADR-018 decided; the code (state core, API, desk, attestations) already waits  |
| 3   | **PERFORMANCE program (MISSION-017+, in flight)**: measure → gate → optimize; see the performance block below                                                                                                                                                                                                                                                                                                                                                                                       | 🤖                     | Ordered by the Oracle 2026-08-16 ("céntrate en performance")                   |
| 4   | **Deploy-day debts**: consent banner + analytics transport decision (D12 — today memory transport, nothing leaves) + 3 `[PENDING]` legal fields                                                                                                                                                                                                                                                                                                                                                     | 🧬 decision, 🤖        | Legal fields need Pablo's company data                                         |
| 5   | **Fix the data repo** (data-doctor): 3 thumbnails, 1 VRM binary on R2, 3dprint catalog 404                                                                                                                                                                                                                                                                                                                                                                                                          | 🧬 Pablo               | Small; then re-run the doctor + refresh fixtures                               |
| 6   | **Session Zero design** (D4: Hyperfy infrastructure location unknown) + canon divergences rulings (D24, manual-map)                                                                                                                                                                                                                                                                                                                                                                                 | 🧬 info, then 🔀       | Blocked on locating the Hyperfy side                                           |
| 7   | **Quality debt**: translation QA ja/ko/pt-br (D9), passkey retest on a phone                                                                                                                                                                                                                                                                                                                                                                                                                        | 🧬 hire / 🤖           | Honest gaps recorded in MISSION-008/009                                        |
| 8   | **MISSION-005 — Data dignity narrative** (specced, seed line live)                                                                                                                                                                                                                                                                                                                                                                                                                                  | 🔀                     | Glossary terms first (ADR-012)                                                 |
| 9   | **Decide `apps/com`'s fate**: single placeholder page while everything real lives in `store`. Keep or fold?                                                                                                                                                                                                                                                                                                                                                                                         | 🧬 decision            | Dead weight in CI and knip until decided                                       |
| 10  | Parked sessions in `docs/open-questions.md`: taxonomy revisit, gender-restriction policy (ADR-013), Huly integration (D6), brand visual package (D8), season pricing (D7), L.A.P.→P.A.R. naming (D5)                                                                                                                                                                                                                                                                                                | 🧬🤖                   | Each scoped and referenced                                                     |
| 11  | **MISSION-030 — numinia.store SEO migration** (301 → numinia.com + Search Console change of address + legacy Vercel shutdown + SIWE `AUTH_DOMAIN` fix): parked by Oracle order 2026-08-16; the .store renewal date is the clock                                                                                                                                                                                                                                                                     | 🧬 domain/GSC, 🤖 code | Specced in numinia-nwos MIS-084 (ex MISSION-030); two live sites split search signals meanwhile |
| 12  | **⛔ Package-map ADR (canon D-07) — BLOCKING (escalated 2026-08-16)**: the repo is PUBLIC, so the MIT declarations are already a live grant; audit confirms `@numinia/auth` holds authoritative session verification (core) under that grant. The ADR must map every package's side, plan the auth server-side extraction to `apps/` before it grows (same review for `@numinia/state`), and set edge release conditions. Also blocking: paste canon v1.3.0 verbatim into CLAUDE.md (marker placed) | 🧬 ruling, 🤖 ADR      | Gates Phase C and any publish; see open-questions D-07 and ADR-019             |
...
- **Main deploys itself** (since 2026-08-15 deploy day): every green CI on main ships to numinia.com. Therefore: nothing lands on main that should not be public, and main stays green. First `npm publish` of any `@numinia/*` package is a one-way stop-and-ask door: `private: true` holds as a preventive brake until the package-map ADR (NEXT #12) signs which packages are the publishable MIT edge — publication of the edge is the canon's intent, not an exception to it.
