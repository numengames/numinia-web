...
| 4   | **Write-path ADR** — how the platform writes (data repo PRs? API? user store?). Unblocks: banning/promoting users, asset upload/edit, sheet sync to a session, Session Zero recording                | 🧬🤖 session              | The single biggest unlock for the game half of the platform                  |
