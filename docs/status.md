...
| 12  | **Package-map ADR (canon queue D-07, priority raised 2026-08-16)**: which `@numinia/*` packages are publishable MIT edge vs unpublishable core, release order and criteria. **Gates Phase C (SPDX headers) and the first `npm publish`** — five packages sit braked (`private: true`) with no signed list of which get released and when | 🧬 ruling, 🤖 ADR      | Ordered BEFORE Phase C by the Oracle; see ADR-019 consequences                 |
