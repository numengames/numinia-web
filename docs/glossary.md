...
> **For humans.** The naming authority: every domain term, ES canonical ↔ EN, with its code identifier.
>
> **Epistemic value.** Resolves what things are called — the single arbiter when names conflict (ADR-012).
> **Pragmatic value.** No domain name enters code without passing through here first; changes require Oracle ratification.
> **In the system.** Observes: RPG manual, seminal corpus. Regulates: all identifiers and copy. Coupled to: packages/domain, CLAUDE.md.
>
...
| ----------- | --------------- | -------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Artisans    | Proyectistas    | Projectors     | `projectors`     | ⚠️ Alternative EN: "Project Designers" (_Role structure_). Constitution v0.2.0 uses "Projectors" — proposed canonical.                                                              |
...
> ⚠️ Constitution §Session Zero says "Thought + Knowledge" for the first threshold; the seminal _About Session Zero_ says **Culture + Wisdom** (used here). CLAUDE.md needs a one-line correction.
...
| Idiolecto → Idiolect (`idiolect`)    | _(free text)_ | _(free text)_ | —               | Per character  |

---

## Open items for the Oracle (⚠ summary)

1. §3 — House `projectors`: "Projectors" (constitution) vs "Project Designers" (_Role structure_). **Proposed: Projectors.**
