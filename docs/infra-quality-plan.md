...
> 8/8 Gherkin, 1/1 e2e — green, but green _only when someone remembers to run it_.
...
2. **CI is a single shallow job** that has never run (no remote): no e2e, no license gate (the legacy _had_ one blocking GPL/AGPL — relevant until D11 is decided), no coverage artifacts, no workflow lint.
