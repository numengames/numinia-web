...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------------------- |
| D1  | ~~**Progressive auth Web2→Web3**~~ **RESOLVED 2026-08-15** — session held; the decision is enforced in `packages/auth`. Follow-ups: D13, D14.                                                                                    | Closed                              | —                   |
...
### D-07 (canon queue) — Package map: publishable edge vs unpublishable core — ⛔ BLOCKING (escalated 2026-08-16, same day)

The conceptual boundary is closed (ADR-019: decides → core/AGPL, shows → edge/MIT), but the exact map of which `@numinia/*` package falls on which side was queued in the canon (D-07) and was explicitly due **before the first publish**. Since 2026-08-16 all five packages sit braked (`private: true`) with no signed list of which get released, in what order, under what criteria. **Escalated from raised to BLOCKING** the same day: the repo went PUBLIC, and a public repo with `license: MIT` declarations is already a live grant — `private: true` only stops `npm publish`, not the offer. The audit confirmed the damage vector is real: **`@numinia/auth` contains authoritative session verification** (`verifySessionToken`, rotation, attestations, single-use nonces) — core by the canon's own frontier, granted under MIT in the published snapshot. The snapshot cannot be ungranted; the containment is forward-looking: **the authoritative server side of `@numinia/auth` must move into `apps/` (AGPL) before it grows further**, leaving at most neutral primitives (encoding, boundary constant) at the MIT edge. Run the same review over `@numinia/state` as part of this ADR. Deliverable: the package-map ADR naming each package's side, the auth (and state, if affected) extraction plan, and the release conditions for the true edge. Gates Phase C and any first publish.
