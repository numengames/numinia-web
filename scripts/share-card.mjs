...
const SITE = {
  kicker: 'NUMEN GAMES · THE GAME',
  name: 'Numinia',
  line: 'A world across three centuries. Its chronicle, its material culture — CC0 — and the game.',
  domain: 'numinia.com',
};

const OUT = path.join(root, 'apps', 'store', 'public');
const FONTS = path.join(root, 'packages', 'ui', 'src', 'assets', 'fonts');
const KHEPRI = path.join(root, 'packages', 'ui', 'src', 'assets', 'brand', 'Khepri_Logo.svg');
...
function check() {
  ...
  if (missing.length) {
    console.error(
      `share-card --check: missing in public/: ${missing.join(', ')} — run node scripts/share-card.mjs`,
    );
    process.exit(1);
  }
...
if (process.argv.includes('--check')) {
  ...
  console.log(
    `share-card: ${SITE.domain} — favicon.svg, favicon.png 32, apple-touch-icon.png 180, og-card.png ${w}×${h} (${(card.length / 1024).toFixed(0)} KB)`,
  );
}
